...
Notebook fix: `notebooks/scvi_pretraining.ipynb`, cells 3 to 4, commit `71f8bcc`.
...
**CUDA and driver mismatch.** `pip install scvi-tools` pulled the newest PyTorch build, compiled for CUDA 13.0. The pod's driver only supported CUDA 12.8. Training crashed immediately with a clear error. Fixed by reinstalling PyTorch pinned to a CUDA 12.1 build (`pip install torch --index-url https://download.pytorch.org/whl/cu121`), which is backward compatible with the 12.8 driver. Lesson for next time: always check `nvidia-smi` for the driver's CUDA version before installing anything, and pin PyTorch to match before installing other packages.

**GPU utilization stuck at 7 percent.** Training worked but was barely touching the A40, 7 percent utilization, 413MB of 46GB VRAM used, and an estimated 38 minutes for 100 epochs despite renting a GPU specifically to avoid slow runs. The cause was scVI's default batch size of 128, which meant about 1,300 tiny batches per epoch, each too small to saturate the GPU's parallelism. Setting `batch_size=4096` dropped this to about 40 batches per epoch. Result: GPU utilization jumped to 48 percent, and 30 epochs finished in 66 seconds, roughly an 11x speedup from one parameter.
...
Two things stand out. First, correcting the HVG selection method alone raised AUC from about 0.70 (Fano factor, laptop) to about 0.76 (proper `seurat_v3`, full RAM), a bigger jump than anything scVI pretraining produced. The earlier memory constrained shortcut cost real signal. Second, and more importantly, **scVI's pretrained latent space did not outperform raw genes. It underperformed by about 3 to 4 AUC points.** The hypothesis going into tonight was that pretraining on 166k unlabeled cells would learn a representation that separates biological signal from technical noise better than raw expression values. That did not happen here.

![Honest AUC, raw genes versus scVI latent](figG_runpod_auc_comparison.png)

This is a real, negative result, not a failure of execution. A better feature representation cannot manufacture a disease signal that is not there in sufficient strength, and the pseudobulk analysis below independently confirms the signal is genuinely weak at this sample size.

To see this directly rather than infer it from a single AUC number, here is the UMAP projection of the scVI latent space itself:

![scVI latent space UMAP](figI_scvi_umap.png)

Cells from different donors are well mixed in the latent space, which means the batch correction did its job and the model is not simply encoding patient identity. But DCM and ACM cells are also heavily intermixed with each other. There is no visible region of the latent space that belongs mostly to one disease. This is consistent with the AUC result: the representation is clean with respect to technical noise, it just does not contain a strong disease axis to find.

**Pseudobulk differential expression, full 32,383 gene panel, no pre-filtering:**

FDR under 0.05: 0 genes. FDR under 0.10: 0 genes.

![Pseudobulk volcano plot, confirmed on the full atlas](figJ_pseudobulk_confirmed.png)

Identical result to the laptop run, same top genes (VTA1, PDGFB, ALG10B), nearly identical p-values. This confirms the earlier zero significant genes finding was not an artifact of the memory constrained pipeline. It is a genuine consequence of having 8 ACM donors. PDGFB remains the most biologically plausible lead, a fibrosis signaling gene, higher in ACM, consistent with ACM's fibro fatty pathology, despite not surviving correction.
...
![Honest AUC by cardiomyocyte substate](figH_cell_substate_auc.png)

vCM3.0 classifies noticeably better than the whole population average. This is a genuinely new lead. DCM and ACM signal is not spread evenly across cardiomyocyte substates, it is concentrated in some more than others. Worth investigating what distinguishes vCM3.0 biologically, which genes define it, what functional state it represents, as a next step, rather than continuing to treat all cardiomyocytes as one bucket.
...
4. Cell substate matters. vCM3.0 is the most promising lead for follow up. Either the DCM and ACM signal genuinely lives there, or it is worth checking whether vCM3.0 has a different sex or genotype composition than the rest of the cohort before reading too much into it.
